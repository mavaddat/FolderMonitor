...
namespace FolderMonitor.UI
{
    ...
    /// </summary>
    public class LogFileReader : IEnumerable<string>
    {
        ...
        public LogFileReader(Func<Stream> streamSource)
        ...
        public LogFileReader(string filename, Encoding encoding)
            : this(() => File.Open(filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)/* File.OpenRead(filename)*/, encoding)
        {
        }
        ...
        public LogFileReader(Func<Stream> streamSource, Encoding encoding)
            : this(streamSource, encoding, DefaultBufferSize)
        {
        }

        internal LogFileReader(Func<Stream> streamSource, Encoding encoding, int bufferSize)
        {
